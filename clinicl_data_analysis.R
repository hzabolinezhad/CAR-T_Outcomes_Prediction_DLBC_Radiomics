...
#' Visualize correlation heatmap with rounded values
#'
...
visualize_correlation <- function(data) {
  ...
  highly_correlated_pairs <- subset(melt(cor_matrix), value > 0.8 & Var1 != Var2)
...
#'
#' This function creates a baseline logistic regression model using the final selected variables and evaluates its performance on both training and testing datasets.
#'
#' @param final_vars A vector containing the names of the final selected variables.
...
#' 
#' @return A matrix containing the performance statistics of the baseline model on both training and testing datasets.
#'
#' @examples
#' \dontrun{
#' statistics <- create_baseline_model(final_vars, train_data, test_data, target_col_name, outputdir = "./output")
#' }
#' 
#' @export
create_baseline_model <- function(final_vars, train, test, y, outputdir) {
  ...
  if (!is.null(length(final_vars))) {
    ...
    base_line_model <- lrm(formula = frm, x = TRUE, y = TRUE, data = train)
    ...
    # statistics["train", "Sensitivity"] <- train_conf_matrix$byClass['Sensitivity']
    ...
    # statistics["Test", "Sensitivity"] <- test_conf_matrix$byClass['Sensitivity']
